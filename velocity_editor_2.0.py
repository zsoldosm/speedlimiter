...
limit = 1.1   # [m/s^2]
Debug = False   # Belső függvények értékeinek kiiratása Terminálba (True --> Kiiratja, False --> Nem iratja ki)
#-------------------------------------------------------------------
...
def delta_speed_l(filename, limit):
    """Mintavételezési pontok közötti limitált sebesség - különbségek kiszámítása."""
    dsl = []
    for i in range(1, len(limiter(filename, limit))):
        dsl.append((limiter(filename, limit)[i])-limiter(filename, limit)[i-1])
...
def del_accelerate(filename):
    ...
    dv = delta_speed(filename)
    dt = delta_time(filename)
    acc = []
...
def limiter(filename, limit):
    ...
    for i in range(0, len(v)-1):
        if (((v[i+1] - v_lim[i])/dt[i]) >= (limit * 3.6)):
            v_lim[i+1] = round((v_lim[i] + (limit * 3.6 * dt[i])), 4)
    ...
    for j in range(len(v)-1, 0, -1):
        if ((v_lim[j-1] - v_lim_2[j]) >= (limit * 3.6 * dt[j-1])):
            v_lim_2[j-1] = round((v_lim_2[j] + (limit * 3.6 * dt[j-1])), 4)
        else: v_lim_2[j-1] = v_lim[j-1]
    
    return v_lim_2

def plot_vt(filename, limit):
    """Sebesség függvény (eredeti és limitált) kirajzoltatása."""

    x1 = time(filename)
    x2 = time(filename)

    y1 = data(filename)["velocity"]
    y2 = limiter(filename, limit)
...
def plot_at(filename, limit):
    """Gyorsulás függvény (eredeti és limitált) kirajzoltatása."""

    x1 = time(filename)[:-1]
    x2 = time(filename)[:-1]

    y1 = del_accelerate(filename)
    y2 = del_accelerate_l(filename, limit)
...
lim = np.asarray(limiter(filename, limit), order='F')
df1 = pd.DataFrame({
...
lima = np.asarray(del_accelerate_l(filename, limit), order='F')
df2 = pd.DataFrame({
    'Original': ora,
    'Limited': lima
})
...
if (Debug == True):
    ...
    print("\nKét pont közötti limitált sebesség különbség [km/h]:\n")
    print(delta_speed_l(filename, limit))
    print("\nAdott pontban limitált gyorsulás értékek [m/s^2]:\n")
    print(del_accelerate_l(filename, limit))
